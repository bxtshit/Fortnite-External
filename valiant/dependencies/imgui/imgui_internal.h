...
struct ImGuiContext
{
    ...
    ImGuiID                 NavActivateId;                      // ~~ (g.ActiveId == 0) && (IsKeyPressed(ImGuiKey_Space) || is_down(ImGuiKey_Enter) || IsKeyPressed(ImGuiKey_NavGamepadActivate)) ? NavId : 0, also set when calling ActivateItem()
    ImGuiID                 NavActivateDownId;                  // ~~ is_down(ImGuiKey_Space) || is_down(ImGuiKey_Enter) || is_down(ImGuiKey_NavGamepadActivate) ? NavId : 0
...
//#endif
    ...
    IMGUI_API bool              is_down(ImGuiKey key, ImGuiID owner_id);
